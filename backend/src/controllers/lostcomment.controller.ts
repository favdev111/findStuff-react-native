...
import LostComment from "../models/LostComment";
import mongodb from "mongodb";

class LostCommentController {
  public async getItems(req: Request, res: Response): Promise<void> {
    const items = await LostComment.find();
    res.json(items);
  }
  ...
  public async getItem(req: Request, res: Response) {
    try {
      ...
      const item = await LostComment.findOne({
  ...
  public async updateItem(req: Request, res: Response): Promise<any> {
    try {
      const url = req.params.url;
      const updatedItem = await LostComment.findOneAndUpdate(
        { _id: new mongodb.ObjectID(url) },
        req.body,
        {
          new: true
  ...
  public async deleteItem(req: Request, res: Response): Promise<any> {
    try {
      const url = req.params.url;
      const deletedItem = await LostComment.findOneAndDelete(
        { _id: new mongodb.ObjectID(url) },
        req.body
      );
...
export default new LostCommentController();
